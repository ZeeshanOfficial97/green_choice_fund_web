<template>
  <div>
    <solution-list-add-new
      :is-add-new-solution-sidebar-active.sync="isAddNewSolutionSidebarActive"
      @refetch-data="refetchData"
      :category-options="categoryOptions"
    />

    <!-- Filters -->
    <solution-list-filters
      :category-filter.sync="categoryFilter"
      :publish-filter.sync="publishFilter"
    ...
    <b-card no-body class="mb-0">
        ...
        <b-row>
          ...
          <b-col cols="12" md="6">
            <div class="d-flex align-items-center justify-content-end">
              ...
              <b-button
                variant="primary"
                @click="isAddNewSolutionSidebarActive = true"
              >
                <span class="text-nowrap">Add Solution</span>
              </b-button>
            </div>
          </b-col>
        </b-row>
      ...
      <b-table
        ref="refSolutionListTable"
        class="position-relative"
        :items="fetchSolutions"
        responsive
        :fields="tableColumns"
        primary-key="id"
        :sort-by.sync="sortBy"
      ...
      >
        ...
        <template #cell(status)="data">
          <b-badge
            pill
            :variant="`light-${resolveSolutionStatusVariant(data.item.status)}`"
            class="text-capitalize"
          >
            {{ data.item.status }}
          </b-badge>
...
import SolutionListAddNew from "./SolutionListAddNew.vue";
import SolutionListFilters from "./SolutionListFilters.vue";
// Notification
...
export default {
  ...
  setup() {
    ...
    const isAddNewSolutionSidebarActive = ref(false);
  ...
  methods: {
    ...
    confirmDelete(id) {
      this.$swal({
        ...
        if (result.value) {
          store
            .dispatch("app-solution/deleteSolution", {
              id: id,
            })
            .then((response) => {
              this.$toast({
